...
_completion_file="${_cache_dir}/git-system-completion.bash"

# Create the cache directory if it doesn't exist.
mkdir -p "$_cache_dir"

_regenerate_git_completion_cache=false

# Check if cache file exists
if [[ ! -f "$_completion_file" ]]; then
    _regenerate_git_completion_cache=true
else
    # Check if any source completion file is newer than the cache
    for completion in "$bash_completion_dir"/git*; do
        if [[ -f "$completion" && "$completion" -nt "$_completion_file" ]]; then
            _regenerate_git_completion_cache=true
            break
        fi
    done
...
if $_regenerate_git_completion_cache; then
    # Regenerate the combined completion file
    echo "# Generated by git-bar.sh on $(date)" > "$_completion_file"
    for completion in "$bash_completion_dir"/git*; do
        if [[ -f "$completion" ]]; then
            cat "$completion" >> "$_completion_file"
            echo "" >> "$_completion_file" # Add newline for separation
        fi
    done
fi

# Source the cached completion file.

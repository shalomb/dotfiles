...
function cd {
  ...
  # common locations
      ...
      case "$path" in
        ...
        -f)
          # Use gum for better data
          while read -r i; do candidates+=("$i"); done < <(gum dirs --format simple)
        ;;
        ...
        # Projects
        -p)
          while read -r i; do candidates+=("$i"); done < <(gum projects --format simple)
        ;;

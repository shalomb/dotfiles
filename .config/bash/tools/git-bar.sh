...
#! ghb - gh browse
function ghb {
  local repo_url=$(grv | awk '/(fetch)/{print $2}')
  ...
  case "$1" in
    -R) # Repo root
      ...
      ;;
    -a) # Actions
      x-www-browser "$repo_url/actions"
      ;;
    -b) # Branches
      x-www-browser "$repo_url/branches/all"
      ;;
    -C) # Compare
      x-www-browser "$repo_url/compare/$(gbr)..main"
      ;;
    -c) # Commits
      x-www-browser "$repo_url/commits"
      ;;
    -h) # Help
      type -a ghb
      ;;
    -i) # Issues
      ...
      ;;
    -p) # Pulls
      x-www-browser "$repo_url/pulls"
      ;;
    -t) # Tags
      x-www-browser "$repo_url/tags"
      ;;
    -T) # Tree
      shift
      [[ $# < 1 ]] && set -- $(git rev-parse --verify HEAD --short)
      x-www-browser "$repo_url/tree/$*"
      ;;
    *)
      if (( $# < 1 )) || [[ $1 == "." ]]; then
        set -- -b "$(gbr)" '.'
...
#! ghi - gh browse issue/pr # issues
function ghi { gh browse "${*:-.}"; }

#! ghprv - gh pr view -w # pr view
function ghprv { gh pr view --web "$*"; }

#! ghr - gh browse -r # releases
function ghr { gh browse -r "${*:-.}"; }
...
function gfbd {
  [[ $# = 0 ]] && set -- $(gbr)
  command git flow bugfix finish "$@"
}
...
function gfx {
  local commit
  if (($# == 0)); then
    response=$(gl --color=always | fzf --ansi)
    commit=$(awk '{ print $2 }' <<<"$response")
  fi
...
function grc {
  local current_tag="${1:-$(git describe --tags --exact-match --match 'v*.*.*')}"

...
let g:NTreeAutoClose=0
function! s:ensureNTreeSize_au(parm)
  let action=a:parm
  if g:NTreeAutoClose == 1
    if action == "Enter"
      vertical resize 32 
    else
      vertical resize 0
...
function! s:bindMappings()
    ...
    nnoremap <buffer> <silent> <space>  \|:silent exec 'vertical resize '.(winwidth('.') != 31 ? 32 : 64)<CR>
    let bufname=escape(substitute(expand('%:p', 0), '\\', '/', 'g'), ' ')
    exec 'au WinLeave '.bufname.' call s:ensureNTreeSize_au("Leave")'
    exec 'au WinEnter '.bufname.' call s:ensureNTreeSize_au("Enter")'
